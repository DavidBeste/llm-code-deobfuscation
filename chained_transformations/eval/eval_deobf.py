...
def calc_all_metrics(original, obfuscated, deobfuscated, orig_data_suffix, obfs_data_suffix, data_suffix):
#        original_files = [file for file in os.listdir(original) if file.endswith(".c") and not "tigress_out" in file]
        original_files = [(f + orig_data_suffix + ".c", f) for f in function_names]
...
#        print("Obfuscated files")
        ...
        calc_metrics(files=deobfuscated_files, path=deobfuscated, suffix_to_remove=".c", fname="", all_functions=False)


def run_semantical_tests(path, io_path, function_name, orig_data_suffix, data_suffix): # path without file ending
    ...
    if os.path.exists(f"{path}/{function_name}{data_suffix}.exe"):
        inputs = [f for f in os.listdir(io_path + "/" + function_name + orig_data_suffix) if f.endswith(".json")]

        for inp in inputs:
            cmd = (f"{path}/{function_name}{data_suffix}.exe", io_path + "/" + function_name + orig_data_suffix + "/" + inp, f"{path}/{function_name}{data_suffix}/output_{inp.removeprefix('input_')}")
            print(cmd)
...
def main():
    if not args.no_metrics:
        calc_all_metrics(f"{args.original_path}_eval", f"{args.obfuscated_path}_eval", f"{args.deobfuscated_path}_eval", args.orig_data_suffix, args.obfs_data_suffix, args.data_suffix)

    for function_name in function_names:
        run_semantical_tests(f"{args.original_path}_eval_io_test", args.io_path, function_name, args.orig_data_suffix, args.orig_data_suffix)
        run_semantical_tests(f"{args.obfuscated_path}_io_test", args.io_path, function_name, args.orig_data_suffix, args.obfs_data_suffix)
        run_semantical_tests(f"{args.deobfuscated_path}_io_test", args.io_path, function_name, args.orig_data_suffix, args.data_suffix)
