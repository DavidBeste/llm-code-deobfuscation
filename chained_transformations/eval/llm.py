...
# Example: python3 llm.py --model_type deepseek-coder-instruct --eval_model deepseek-ai/deepseek-coder-6.7b-instruct --eval_file datasets/obfuscation_dataset_6_chain_eval_l.txt --eval_out_path datasets/deobfuscated --obfs_data_suffix _7_chain  --data_suffix _7_chain_6144
...
class LocalLLM(LLMBase):
    ...
    def evaluate(self, dataset, dir_name, data_suffix, stop_token=None):
        ...
        for sample in dataset:
            ...
            if os.path.exists(f"{dir_name}_eval/{name}{data_suffix}.json"):
                continue
   #        print("test")
...
#        dataset = prepare_eval_dataset(dataset, args.model_type, args.eval_model, self.tokenizer)
        ...
        for sample in dataset:
            ...
            print(f"{dir_name}_eval/{name}{data_suffix}.c")
    ...
    def build_evaluatable_code(self, dataset, dir_name, obfs_data_suffix, data_suffix):
        ...
        for sample in samples:
            ...
            try:
                ...
                print(f"Error parsing JSON from file {dir_name}_eval/{name}{data_suffix}.json: {e}")
                # write the io wrapper
            ...
            if output == {'error': 'too long'}:
                with open(f"{dir_name}_io_test/{name}{data_suffix}.cpp", "w") as f:
                    f.write("error, too long")
