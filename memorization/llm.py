...
# Example: python3 llm.py --model_type deepseek-coder-instruct --eval_model models/deepseek-coder-instruct-7b-encode_arithmetic/ --eval_out_path datasets/deobfuscated --eval_file datasets/obfuscation_dataset_encode_arithmetic_eval.json --max_tokens 2048 --data_suffix _encode_arithmetic
...
#    print(f"Sample count after {len(samples)}")
    ...
    print(samples[0])
  #  samples = [format_obf_org_pair(*sample.split("<|ORIG|>"), model_type=model_type, model=model_name, tokenizer=tokenizer) for sample in samples]
    samples = [{key: value.split("<|ORIG|>")[0].split("<|OBFS|>")[1]} for sample in samples for key, value in sample.items()]
    print(samples[0])
...
class LocalLLM(LLMBase):
   ...
   #        print("test")
    ...
    def build_evaluatable_code(self, dataset, dir_name, obfs_data_suffix, data_suffix, obfs_path="MemTest/obfuscated_modified_eval/"):
#        self.tokenizer = AutoTokenizer.from_pretrained(self.model_id, trust_remote_code=True)
        seed = 42
...
#        dataset = prepare_eval_dataset(dataset, args.model_type, args.eval_model, self.tokenizer)
        ...
        for sample in dataset:
            ...
            helper_functions = extract_function(obfs_path + fname_orig + obfs_data_suffix + ".c", name, "", False, True, True, extract_helpers=True, extract_only_helpers=True)
            ...
            io_program = correct_function_arguments(io_program, fname_orig, "obfuscated", args.obfs_data_suffix, args.data_suffix, obfs_path="MemTest/obfuscated_modified", deob_path="MemTest/deobfuscated_modified")
   ...
   #        print("test")
    ...
    def build_evaluatable_code(self, dataset, dir_name, obfs_data_suffix, data_suffix):
        ...
        for sample in samples:
            ...
            # extract helper functions from obfuscated, data suffix can differentiate between specific transformations and chains
            helper_functions = extract_function("MemTest/obfuscated_modified_eval/" + fname_orig + obfs_data_suffix + ".c", name, "", False, True, True, extract_helpers=True, extract_only_helpers=True)
