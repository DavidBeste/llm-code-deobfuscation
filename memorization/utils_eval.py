...
def obfuscate_simple(filename, target_function, parameter_count, orig_path, obfs_path, is_eval=False):
...
#        "virtualize" : " --Transform=Virtualize --Functions=" + target_function + " --Transform=CleanUp --CleanUpKinds=names,annotations"# later also region level obfuscation for the training dataset to increase diversity? What about self modifying virtualization?
    ...
    for k in command_templates.keys():
        ...
        if k != "randomize_arguments" or not is_eval:
            if k != "basic":
            ...
            full_cmd = cmd + command_templates[k]
            full_cmd += " " + orig_path + filename + " --out=" + obfs_path + filename.removesuffix(".c") + "_" + k + "_intermediate.c"
            ...
            full_cmd = cmd + command_templates['basic'] + " " + obfs_path + filename.removesuffix(".c") + "_" + k + "_intermediate.c" + " --out=" + obfs_path + filename.removesuffix(".c") + "_" + k + ".c"
...
def check_correctness2(function_name_no_suffix, function_name, path1, path2, no_run=False, verbose=False, input_sample_path="datasets/input_samples/"):
...
#        syntax_broken.append(function_name)
    ...
    inputs = [f for f in os.listdir(input_sample_path + function_name_no_suffix) if f.endswith(".json")]
...
#    print("datasets/input_samples/" + function_name_no_suffix)
    ...
    for i in range(10):
        if not no_run:
            cmd = ["./" + path1 + function_name_no_suffix + ".exe", input_sample_path + function_name_no_suffix + "/input_" + str(i) + ".json", path1 + function_name_no_suffix + "/output_" + str(i) + ".json"]
            print(cmd)
            start = time.time()
            p = subprocess.Popen(cmd)
    ...
    #            runtime_broken.append(function_name_no_suffix)
            ...
            cmd = ["./" + path2 + function_name + ".exe", input_sample_path + function_name_no_suffix + "/input_" + str(i) + ".json", path2 + function_name + "/output_" + str(i) + ".json"]
            start = time.time()
            print(cmd)
            p = subprocess.Popen(cmd)
...
def correct_function_arguments(io_wrapper, f_name, dir_name, obfs_data_suffix, data_suffix, obfs_path="datasets/obfuscated", deob_path="datasets/deobfuscated"):
    print(f"{obfs_path}_io_test/{f_name}{obfs_data_suffix}.cpp")
    print(f"{obfs_path}_eval/{f_name}{obfs_data_suffix}.c")

    with open(f"{obfs_path}_io_test/{f_name}{obfs_data_suffix}.cpp", "r") as f:
        code1 = f.read()

    with open(f"{obfs_path}_eval/{f_name}{obfs_data_suffix}.c", "r") as f:
        func1 = f.read()


    obfuscated_function_name, obfuscated_code = extract_function(f"{obfs_path}_eval/{f_name}{obfs_data_suffix}.c", f_name, "", False, False, False)
  ...
  #  print(f"code2: {code2}")
    ...
    with open(f"{deob_path}_eval/{f_name}{data_suffix}.c", "r") as f:
        func2 = f.read()
...
def check_correct_output(path, function_name, data_suffix):
    pass
