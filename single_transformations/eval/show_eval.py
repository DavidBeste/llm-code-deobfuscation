...
from utils_eval import check_correctness2, compute_deobfuscation_performance, count_elements, get_metrics_function_name, load_metrics, load_metrics_new, get_code
...
#        print(deobfuscated_metrics)
    ...
    if  False and (performance[1] > 1 or performance[1] < 0):
        #print(args.obfuscated_path + "_eval/" + sample + args.obfs_data_suffix + "_metrics.json")
        #print(args.deobfuscated_path + "_eval/" + sample + args.data_suffix[0] + "_metrics.json")
        print("nano " + args.original_path  + "_eval/" + sample + args.orig_data_suffix[0] + "_tigress_canonicalized_function.c ", end="")
        print(args.obfuscated_path + "_eval/" + sample + args.obfs_data_suffix + "_function.c ", end="")
...
metrics = compute_metrics(original_metrics_all, obfuscated_metrics_all, deobfuscated_metrics_all)

# print(f"Individual scores: {metrics}")
print(f"Ratio for bar plots: {np.nanmean(metrics, axis=0)}")
print(f"Standard deviation for bar plots: {np.nanstd(metrics, axis=0)}")
...
# Wrong: print(f"Ratio for standard deviation: {np.std(original_metrics_all, axis=0)/np.std(deobfuscated_metrics_all, axis=0)}")
