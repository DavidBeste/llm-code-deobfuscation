...
for index, function_name in zip(dataset_indices, function_names):
    remove_keywords("datasets/original_eval/" + function_name + ".c")
    os.system("clang -S -fPIC -emit-llvm -O3 datasets/original_eval/" + function_name + ".c -o datasets/original_eval/" + function_name + ".ll")
    os.system(f"clang -shared -fPIC -undefined dynamic_lookup datasets/original_eval/{function_name}.ll -o datasets/original_eval/{function_name}.so")
    print(calculate_metrics_program_from_disk("datasets/original_eval/" + function_name + ".ll", function_name))
    cycl, (n1, n2, N1, N2, vocabulary, program_length, program_volume, difficulty, effort, time, bugs) = calculate_metrics_program_from_disk("datasets/original_eval/" + function_name + ".ll", function_name)
    print(program_length)
    print(cycl)
    save_metrics("datasets/original_eval/" + function_name + "_llvm_metrics.json", function_name, cycl, program_length, vocabulary)

    remove_keywords("datasets/obfuscated_eval/" + function_name + args.obfs_data_suffix + "_function.c")
    os.system("clang -S -fPIC -emit-llvm -O3 datasets/obfuscated_eval/" + function_name + args.obfs_data_suffix + "_function.c -o datasets/deobfuscated_eval/" + function_name + args.obfs_data_suffix + ".ll")
...
#    w = correct_function_arguments(w, function_name, "datasets/deobfuscated_eval", args.obfs_data_suffix, args.data_suffix)
    ...
    os.system("clang -S -fPIC -emit-llvm -O0 datasets/obfuscated_eval/" + function_name + args.obfs_data_suffix + "_function.c -o datasets/obfuscated_eval/" + function_name + args.obfs_data_suffix + ".ll")
    ...
    cycl, (n1, n2, N1, N2, vocabulary, program_length, program_volume, difficulty, effort, time, bugs) = calculate_metrics_program_from_disk("datasets/obfuscated_eval/" + function_name + args.obfs_data_suffix + "_O0.ll", function_name_obfuscated)
